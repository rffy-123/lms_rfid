...
def register():
    if request.method == 'POST':
        ...
        if len(email) < 1 or len(password) < 1:
            return render_template('admin/register.html', error="Email and password are required")

        result = admin_manager.add_admin(email, password)

        if result == "email_exists":
            return render_template('admin/register.html', error="Email already exists")

        
        return render_template('admin/register.html', msg="Admin registered successfully")
...
def search():
	...
	if len(d) >0:
		return render_template("books/views.html", search=True, books=d, count=len(d), keyword=(keyword), g=g, admin=admin)

	return render_template('books/views.html', error="No books found!", keyword=(keyword))
